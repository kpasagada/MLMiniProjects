...
for k in k_array:
    ...
    for i in range(20):
        ...
        # Cluster centers
        cluster_centers = np.empty((k, features_length))

        # Generate random cluster centers
        for num_k in range(k):

            cluster_centers[num_k] = np.random.choice(np.arange(-3, 4, 1), features_length)

        # K Means model
        kmeans_model_array.append(KMeans(n_clusters=k, init=cluster_centers).fit(scaled_train_data))
